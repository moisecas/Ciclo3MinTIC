...
public interface MovimientosRepository extends JpaRepository<MovimientoDinero, Integer> {
    ...
    @Query(value = "select * from movimientos where empleado_id= ?1", nativeQuery = true) //?1
    public abstract ArrayList<MovimientoDinero> findByEmpleado(Integer empleado_id);
